...
export async function getImageBase64Cached(
    ...
    if (!forceRefresh) {
        ...
        if (cached && cached.expiresAt > Date.now()) {
            cacheHits++
            console.log(`${logPrefix} ✅ 缓存命中 (${cacheHits}/${cacheHits + cacheMisses})`)
...
async function downloadImageAsBase64(imageUrl: string, logPrefix: string): Promise<string> {
    ...
    console.log(`${logPrefix} 开始下载: ${imageUrl.substring(0, 80)}...`)
    ...
    try {
        ...
        console.log(`${logPrefix} ✅ 下载完成: ${sizeKB}KB, ${duration}ms`)

        return `data:${contentType};base64,${base64}`
    } catch (error: any) {
        const duration = Date.now() - startTime
        console.error(`${logPrefix} ❌ 下载失败 (${duration}ms): ${error.message}`)
        throw error
    }
}
...
export async function preloadImagesParallel(
    ...
    const { logPrefix = '[批量预加载]', maxConcurrency = 5 } = options
    ...
    console.log(`${logPrefix} 开始预加载 ${uniqueUrls.length} 张唯一图片 (原始: ${imageUrls.length} 张)`)
    ...
    console.log(`${logPrefix} 预加载完成: ${successCount}/${uniqueUrls.length} 成功, ${duration}ms`)
...
function cleanupExpiredCache() {
    ...
    if (cleaned > 0) {
        console.log(`[图片缓存] 清理 ${cleaned} 个过期条目，剩余 ${imageCache.size} 个`)
    }
}
