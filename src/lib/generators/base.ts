    ...
    async generate(params: ImageGenerateParams): Promise<GenerateResult> {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                lastError = error
                console.warn(`[Generator] 尝试 ${attempt}/${maxRetries} 失败: ${error.message}`)
    ...
    async generate(params: VideoGenerateParams): Promise<GenerateResult> {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                lastError = error
                console.warn(`[Video Generator] 尝试 ${attempt}/${maxRetries} 失败: ${error.message}`)
        ...
        return {
            success: false,
            error: lastError?.message || '视频生成失败'
        }
    }
