...
export async function submitGeminiBatch(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(referenceImages.length, 14); i++) {
      ...
      if (imageData.startsWith('data:')) {
        ...
        try {
          ...
          logInternal('GeminiBatch', 'WARN', `下载参考图片 ${i + 1} 失败`, { error: e.message })
        }
      } else {
        // 纯 base64
        contentParts.push({
    ...
    const batchJob = await (ai as any).batches.create({
    ...
    const batchName = batchJob.name  // 格式: batches/xxx
    ...
    logInternal('GeminiBatch', 'ERROR', '提交异常', { error: error.message })
    return { success: false, error: `提交异常: ${error.message}` }
  }
}
...
export async function queryGeminiBatchStatus(batchName: string, apiKey: string): Promise<{
  ...
  try {
    ...
    logInternal('GeminiBatch', 'ERROR', '查询异常', { batchName, error: error.message })
    return { status: 'error', completed: false, failed: false, error: `查询异常: ${error.message}` }
  }
}
