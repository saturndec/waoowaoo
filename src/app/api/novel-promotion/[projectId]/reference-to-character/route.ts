...
import { REFERENCE_TO_CHARACTER_PROMPT, IMAGE_TO_CHARACTER_DESCRIPTION_PROMPT, CHARACTER_IMAGE_BANANA_RATIO, addCharacterPromptSuffix, ART_STYLES } from '@/lib/constants'
...
export const POST = apiHandler(async (
    ...
    console.log(`[Project Reference to Character] 参考图数量: ${allReferenceImages.length}`)
    ...
    const imageModel = (preferences as any)?.characterModel
    const analysisModel = preferences?.analysisModel
    ...
    if (!imageModel) {
        ...
        throw new ApiError('MISSING_CONFIG', { message: '请先在设置页面配置角色图片模型', errorCode: 'IMAGE_MODEL_NOT_CONFIGURED' })
    }

    console.log(`[Project Reference to Character] 开始转换，项目: ${projectId}, 图片模型: ${imageModel}, 仅提取: ${!!extractOnly}, 自定义描述: ${!!customDescription}`)

    // 🔥 模式1：仅提取描述（反推提示词）
    if (extractOnly) {
        if (!analysisModel) {
            throw new ApiError('MISSING_CONFIG', { message: '请先在设置页面配置分析模型', errorCode: 'ANALYSIS_MODEL_NOT_CONFIGURED' })
        }

        console.log('[Project Reference to Character] 仅提取描述模式')
        ...
        console.log(`[Project Reference to Character] 描述提取完成: ${extractedDescription?.substring(0, 50)}...`)
    ...
    const generateSingleImage = async (index: number): Promise<string | null> => {
        try {
            console.log(`[Project Reference to Character] 生成第 ${index + 1}/3 张图片`)
            ...
            if (result.async && result.requestId && result.endpoint) {
                console.log(`[Project Reference to Character] 第 ${index + 1} 张是异步任务，开始轮询: ${result.requestId}`)
                ...
                for (let attempt = 0; attempt < maxPollAttempts; attempt++) {
                    ...
                    if (status.completed && status.resultUrl) {
                        ...
                        console.log(`[Project Reference to Character] 第 ${index + 1} 张异步任务完成: ${finalImageUrl?.substring(0, 50)}...`)
                        break
                    }
                    if (status.failed) {
                        console.error(`[Project Reference to Character] 第 ${index + 1} 张异步任务失败:`, status.error)
                        return null
                    }
                    if (attempt % 10 === 0) {
                        console.log(`[Project Reference to Character] 第 ${index + 1} 张轮询中... (${attempt}/${maxPollAttempts})`)
                    }
                }
            }

            if (!result.success || !finalImageUrl) {
                console.error(`[Project Reference to Character] 第 ${index + 1} 张生成失败:`, result.error)
                return null
            }

            // 下载并添加标签
            const imgRes = await fetchWithTimeoutAndRetry(finalImageUrl, {
                logPrefix: `[Project Reference to Character 图片 ${index + 1}]`
            })
            ...
            // 上传到 COS
            const key = generateUniqueKey(`proj-ref-char-${projectId}-${Date.now()}-${index}`, 'jpg')
            const cosKey = await uploadToCOS(processed, key)
            console.log(`[Project Reference to Character] 第 ${index + 1} 张上传成功: ${cosKey}`)
            return cosKey
        } catch (error: any) {
            console.error(`[Project Reference to Character] 第 ${index + 1} 张处理失败:`, error.message)
            return null
        }
    }
...
只返回描述文字，不要有其他内容。`
    ...
    if (successfulCosKeys.length === 0) {
        console.error('[Project Reference to Character] 所有图片生成都失败')
        if (isBackgroundJob && appearanceId) {
            await prisma.characterAppearance.update({
                where: { id: appearanceId },
                data: { generating: false }
    ...
    console.log(`[Project Reference to Character] 生成完成: ${successfulCosKeys.length}/3 张成功`)
    ...
    if (isBackgroundJob && appearanceId) {
        ...
        console.log(`[Project Reference to Character] 后台任务完成: ${successfulCosKeys.length} 张图片`)
