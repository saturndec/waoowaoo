  ...
  return (
    <div className="p-4 space-y-2">
      <div className="flex items-center justify-between text-xs">
        <span className="px-2 py-0.5 bg-primary/10 text-primary rounded font-medium">{panel.textPanel?.shot_type || t('panelCard.unknownShotType')}</span>
        {panel.textPanel?.duration && <span className="text-muted-foreground">{panel.textPanel.duration}{t('promptModal.duration')}</span>}
      </div>

      <p className="text-sm text-muted-foreground line-clamp-2">{panel.textPanel?.description}</p>
        ...
        {layout.isLastFrame && layout.prevPanel && !layout.isLinked ? (
          ...
          <>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-muted-foreground">{t('promptModal.promptLabel')}</span>
              {!promptEditor.isEditing && (
                <button onClick={promptEditor.handleStartEdit} className="text-muted-foreground hover:text-primary transition-colors p-0.5">
            ...
            {promptEditor.isEditing ? (
              <div className="relative mb-3">
                <textarea
                  ...
                  autoFocus
                  className="w-full text-xs p-2 pr-16 border border-primary/40 rounded-lg bg-card text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary resize-none"
                  rows={3}
                  placeholder={t('promptModal.placeholder')}
                />
                <div className="absolute right-1 top-1 flex flex-col gap-1">
                  <button onClick={promptEditor.handleSave} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-primary text-white rounded">{promptEditor.isSavingPrompt ? '...' : t('panelCard.save')}</button>
                  <button onClick={promptEditor.handleCancelEdit} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-muted text-muted-foreground rounded">{t('panelCard.cancel')}</button>
                </div>
              </div>
            ) : (
              <div onClick={promptEditor.handleStartEdit} className="text-xs p-2 border border-border rounded-lg bg-muted text-muted-foreground cursor-pointer">
                {promptEditor.localPrompt || <span className="text-muted-foreground italic">{t('panelCard.clickToEditPrompt')}</span>}
              </div>
            )}
              ...
              return (
                <div className="mt-2 flex items-center gap-2">
                  <button
                    ...
                    className="flex-shrink-0 min-w-[120px] py-2 px-3 text-sm font-medium rounded-lg shadow-sm transition-all disabled:opacity-50 bg-primary text-white"
                  >
              ...
              <>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-shrink-0 min-w-[90px] py-2 px-3 text-sm font-medium rounded-lg shadow-sm transition-all disabled:opacity-50 bg-primary text-white"
                  >
                  ...
                  <div className="mt-2">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-1.5 text-xs rounded-lg transition-all flex items-center justify-center gap-1 bg-primary text-white disabled:opacity-50"
                      >
                      ...
                      {(taskStatus.isLipSyncTaskRunning || panel.lipSyncVideoUrl) && voiceManager.hasMatchedAudio && (
                        <button onClick={lipSync.handleStartLipSync} disabled={lipSync.executingLipSync} className="flex-shrink-0 px-3 py-1.5 text-xs rounded-lg bg-amber-700 text-white">
                          {t('panelCard.redo')}
                        </button>
                      )}
                    </div>
                    ...
                    {voiceManager.audioGenerateError && (
                      <div className="mt-1 p-1.5 bg-destructive/10 border border-destructive/30 rounded text-[10px] text-destructive">
                        {voiceManager.audioGenerateError}
                      </div>
                    )}
                    ...
                    {voiceManager.localVoiceLines.length > 0 && (
                      <div className="mt-2 space-y-1">
                        {voiceManager.localVoiceLines.map((voiceLine) => {
                          ...
                          return (
                            <div key={voiceLine.id} className="flex items-start gap-1.5 p-1.5 bg-muted rounded text-[10px]">
                              {voiceLine.audioUrl ? (
                                <button
                                  onClick={(event) => {
                                  ...
                                  className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center transition-colors bg-muted"
                                ...
                                <button
                                  ...
                                  className="flex-shrink-0 px-1.5 py-0.5 bg-primary text-white rounded disabled:opacity-50"
                              ...
                              <div className="flex-1 min-w-0">
                                <span className="text-muted-foreground">{voiceLine.speaker}: </span>
                                <span className="text-muted-foreground">&ldquo;{voiceLine.content}&rdquo;</span>
                              </div>
                            </div>
                          )
                        })}
