...
export default function NovelInputStage({
  ...
  // 获取当前选中的风格信息
  const currentArtStyle = ART_STYLES.find(s => s.value === artStyle) || ART_STYLES[0]
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {episodeName && (
        <div className="text-center py-2">
          ...
          <div className="text-sm text-slate-400 mt-1">{t("storyInput.editingTip")}</div>
        </div>
      )}
...
清晨，阳光透过窗帘洒进房间。小明揉着惺忪的睡眼从床上坐起，看了一眼床头的闹钟——已经八点了！他猛地跳下床，手忙脚乱地开始穿衣服...`}
            className="w-full h-80 bg-white/50 border border-slate-200/50 rounded-xl px-4 py-3 text-base resize-none outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 transition-all custom-scrollbar placeholder:text-slate-400"
            disabled={isGenerating || isTransitioning}
          />

          {/* 资产库引导提示 */}
          <div className="mt-5 p-4 bg-white/60 backdrop-blur border border-slate-200/60 rounded-xl">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-slate-700 mb-1">{t("storyInput.assetLibraryTip.title")}</div>
      ...
      {/* 画面比例与视觉风格配置 */}
      <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/50 shadow-sm p-6 relative z-10">
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/50 shadow-sm p-6">
        {/* 旁白开关 */}
        {onEnableNarrationChange && (
          <div className="flex items-center justify-between p-4 bg-slate-50/50 rounded-xl mb-6">
            <div className="flex items-center gap-3">
              <span className="text-2xl">🎙️</span>
              <div>
                <div className="font-medium text-slate-800">{t("storyInput.narration.title")}</div>
                <div className="text-xs text-slate-500">{t("storyInput.narration.description")}</div>
              </div>
            </div>
            <button
              onClick={() => onEnableNarrationChange(!enableNarration)}
              className={`relative w-14 h-8 rounded-full transition-colors ${enableNarration ? 'bg-blue-500' : 'bg-slate-300'
                }`}
            >
              <span
        ...
        <button
          onClick={onNext}
          disabled={!hasContent || isGenerating || isTransitioning}
          className="w-full py-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:from-blue-600 hover:to-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-lg transition-all flex items-center justify-center gap-2"
        >
          {isTransitioning ? (
            <>
              ...
              <span>{t("storyInput.creating")}</span>
            </>
          ) : (
            <>
